...
public class StandardPSO extends AbstractParticleSwarmOptimization<DoubleSolution, DoubleSolution> {
  ...
  private List<DoubleSolution> auxSwarm =null;
   ...
   * @param evaluator
   */
  public StandardPSO(DoubleProblem problem, int objectiveId, int swarmSize, int maxIterations,
   ...
   * @param evaluator
   */
  public StandardPSO(DoubleProblem problem, int swarmSize, int maxIterations,
                         int numberOfParticlesToInform, SolutionListEvaluator<DoubleSolution> evaluator,List<DoubleSolution> swarm,
      List<Double> asp) {
  ...
  public List<DoubleSolution> createInitialSwarm() {
    List<DoubleSolution> swarm  = null;//= new ArrayList<>(swarmSize);
    if(auxSwarm!=null){
      swarm =auxSwarm;
      for (int i = 0; i < swarmSize; i++) {
        positionInSwarm.setAttribute(swarm.get(i),i);
      }
      }else {
